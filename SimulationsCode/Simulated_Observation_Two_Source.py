...
for i in range(numchunks):
    ...
    theta_radians_s2 = np.linspace(theta_current+theta_offset_s2, theta_current + chunk_size*wearth+theta_offset_s2, n_samples,endpoint=False)
    
    tau_geo_s2 = (baseline / c) * np.sin(theta_radians_s2)

    i_s1_dropped_power= np.sqrt(var_s1*(1-0.1*theta_current*180/np.pi))*i_s1
    q_s1_dropped_power= np.sqrt(var_s1*(1-0.1*theta_current*180/np.pi))*q_s1

    i_s2_dropped_power= np.sqrt(var_s2*(1-0.1*(theta_current+theta_offset_s2)*180/np.pi))*i_s2
    q_s2_dropped_power= np.sqrt(var_s2*(1-0.1*(theta_current+theta_offset_s2)*180/np.pi))*q_s2
...
np.savez(f"Two_source_simulated_results_arrays_1s_chucnk_size_baseline_12m_Isntr_Delay_0_102us.npz", 
